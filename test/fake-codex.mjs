...
reader.on("line", (line) => {
  ...
  if (method === "initialize") {
    ...
    send({ id, result: { data: [
      { id: "thread-1", name: "Fixture thread", cwd: process.cwd(), model: "gpt-6-astra", reasoningEffort: "medium", updatedAt: 1_800_000_000, status: { type: "idle" } },
    ...
    const settings = threadSettings.get(params.threadId) || { model: "gpt-6-astra", effort: "medium" };
    send({ id, result: { thread: { id: params.threadId, name: "Fixture thread", cwd: process.cwd(), model: settings.model, reasoningEffort: settings.effort, status: { type: "idle" }, turns: [] } } });
    ...
    threadSettings.set(threadId, { model: params.model || "gpt-6-astra", effort: "medium" });
    send({ id, result: { thread: { id: threadId, name: null, cwd: params.cwd, model: params.model || "gpt-6-astra", reasoningEffort: "medium", status: { type: "idle" } } } });
    ...
    const previous = threadSettings.get(params.threadId) || { model: "gpt-6-astra", effort: "medium" };
    const settings = { model: params.model || previous.model, effort: params.effort || previous.effort };
    threadSettings.set(params.threadId, settings);
    send({ id, result: {} });
    send({ method: "thread/settings/updated", params: { threadId: params.threadId, threadSettings: settings } });
    ...
    if (process.env.FAKE_CODEX_REQUIRE_MODEL_SETTINGS === "1" && (params.model !== "gpt-6-astra" || params.effort !== "high")) {
      send({ id, error: { code: -32602, message: "turn/start requires the selected model settings" } });
      return;
    }
    turnId += 1;
